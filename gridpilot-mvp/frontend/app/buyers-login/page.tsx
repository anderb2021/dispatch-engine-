...
import { loginBuyerDemo } from "@/lib/auth";
...
export default function BuyersLoginPage() {
  ...
  function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const ok = loginBuyerDemo(username, password);
    if (!ok) setError("Invalid buyer credentials.");
  }
      ...
      <section className="mx-auto max-w-md px-6 py-10">
        <div className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
          <div className="mb-6 flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-semibold text-slate-950">Buyer Login</h1>
              <p className="text-sm text-slate-500">Sign in before opening buyer dashboard</p>
            </div>
          </div>
