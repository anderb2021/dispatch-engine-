...
export default async function DashboardPage() {
  ...
  const [
    ...
    await Promise.all([
      ...
      supabase
        ...
        .select(
          "auto_flex_enabled,manual_override_enabled,reward_notifications_enabled,minimum_battery_floor,max_delay_hours,payout_method"
        )
        .eq("user_id", user.id)
        .limit(1),
    ]);
  ...
  return (
    <UserDashboard
      ...
      initialPreferences={preferenceRows?.[0] ?? null}
    />
  );
}
